...
//const String MQTT_SERVER = "yourmqttserver";
...
//  timerAlarmWrite(timer2, 400000, true);
 ...
 //frequency discovered empirically
  ledcSetup(0, 500, 8); // Recebe: (canal, frequência, resolução)
}
...
void loop() {
 /* if (!client.connected()) {
  ...
  /*long now = millis();
    ...
    char rpmString[8];
    dtostrf(encoder1.RPM, 1, 2, rpmString);
    client.publish("RPM atual", rpmString);
  }
  */
  if (interrupt2flag) {
  //Serial.print("erro:");
